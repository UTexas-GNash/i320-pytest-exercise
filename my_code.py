...
def fix_phone_num(phone_num_to_fix):
  if len(phone_num_to_fix) != 10:
    raise ValueError("Can only format when exactly 10 digits long")
  if not phone_num_to_fix.isdigit():
    raise ValueError("Can only format when containing only digits")
...
def test_fix_phone_num_too_long():
